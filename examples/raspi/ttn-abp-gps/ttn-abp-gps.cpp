 ...
 *
 * Fork Psophometric : to send gps coordinates payload. 
 *
 ...
 *
 * This example sends a valid LoRaWAN packet with payload {lat, lon},
...
#include <vector>          // split() function
#include <functional>      // split() function
#include <iostream>        // split() function
using namespace std;       // split() function
...
void do_send(osjob_t* j) {
    ...
    if (LMIC.opmode & OP_TXRXPEND) {
        ...
        cin >> s;
        getSystemTime(strTime , sizeof(strTime));
        cout << ("%s", strTime) << " - " << s << endl;
        split(s, ',', v);
        float float_latitude = ::atof(v[0].c_str());
        float float_longitude = ::atof(v[1].c_str());

        // https://www.thethingsnetwork.org/forum/t/best-practices-when-sending-gps-location-data/1242/13
        uint8_t mydata[6];
